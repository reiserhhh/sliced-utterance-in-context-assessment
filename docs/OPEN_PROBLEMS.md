...
| OP-7 | Dialogue-register transfer unvalidated (clinical target is dialogue; all evidence is posts/essays) | BLOCKING for clinical claims; ENHANCE for methods paper | Pilot: N~30-60, 2 sessions each, fixed-prompt AI dialogue (Rulebook G); rerun PRED-1/B3-style checks on subject turns; measure assistant echo rate | Design ready (Rulebook G); needs data collection |
| OP-8 | English-only scorer; Japanese clinical deployment impossible | BLOCKING for JP deployment; ENHANCE for methods paper | Build JP lexicon set with equivalence protocol (translate -> independent back-check -> within-language re-validation per Rulebook D1) | Not started |
