...
## Dated additions (2026-08-19, thirty-second note — X1; append-only)

| # | era / leg | where recorded | one-line description | rule it paid for | family |
|---|---|---|---|---|---|
| #85 | X-line / M4-X1 | `docs/SUICA_M4_X_EXPRESSION_RESPONSE_PLAN.md — X1 outcome + planner adjudication` | The estimator's zero point was never verified on the REALIZED design geometry: double-centering leaks mains on a fill-0.0018 grid (null world read share 0.0458 = 2.3× the TRACE boundary, R 0.45, the permutation band BELOW the artifact); the zero-reference is not bootstrap-stable; "no support floor needed" was backwards — the estimator needs cross-author support even when the estimand needs no semantic floor. | none (convention: completeness-dependent estimators carry a zero-point gate on the realized skeleton — real incidence, synthetic signal — verified under bootstrap resampling; support conditioning (fill, units-per-condition, singletons, connectivity) is censused and pinned as PRIMARY design; nulls hold fixed everything the estimand claims removed) | zero-point-on-design |

## Dated additions (2026-08-19, thirty-third note — X1b; append-only)

| # | era / leg | where recorded | one-line description | rule it paid for | family |
|---|---|---|---|---|---|
| #86 | X-line / M4-X1b | `docs/SUICA_M4_X_EXPRESSION_RESPONSE_PLAN.md — X1b outcome + planner adjudication` | The gate bundled routing and descriptive clauses (a solved-mechanism descriptive bias stopped a leg whose routing machinery is certified exact); "author main share" was actually the MARGINAL share (bias design-predicted +0.0191 vs +0.0220 observed); the FE's dimension loss (factor 0.8538) was unpinned. | none (convention: gates separate ROUTING clauses (A1-stopping) from DESCRIPTIVE clauses (failure annotates with #67 dual stamp, never stops); budget components name marginal-vs-main exactly and marginals are gate-scored against MARGINAL targets; projection df corrections pinned, corrected routes, raw co-reported) | gate-granularity |

## Dated additions (2026-08-19, thirty-fourth note — X1c; append-only)
...
## Dated additions (2026-08-22, thirty-ninth note — cross-level theory audit; append-only)

| # | era / leg | where recorded | one-line description | rule it paid for | family |
|---|---|---|---|---|---|
| #94 | X-line / measurement-series bridge | `docs/SUICA_MEASUREMENT_SERIES_BRIDGE.md`, `docs/SUICA_JPA_FEED.md` | Common physical units were incorrectly promoted to psychometric measurement invariance, and a difference between two registered linear slopes was promoted to full process ergodicity. X5 itself contains the counterwarning: R1/R4 early and late within-person slopes disagree, so stationarity is unresolved even where the aggregate contrast is precise. | none (convention: physical units license COMMENSURABILITY only; `Delta_erg` is retained as a historical artifact name but interpreted as a PROJECTION-SPECIFIC CROSS-LEVEL TRANSPORT GAP; full invariance and ergodicity require separate empirical conditions) | inference-scope |
| #95 | X-line / X3 synthesis | `reports/SUICA_M4_X3_TRAIT_JOIN_REPORT.md`, `docs/SUICA_JPA_FEED.md` | The projected r=0.019 resolution was quoted as if realized, repeated overlapping PANDORA analyses were called independent replications, and the registered multivariate non-detection was generalized to `trait-mute everywhere` despite exploratory per-trait cells up to abs(r)=0.141. | none (convention: null claims print coordinate-specific REALIZED bands; overlapping same-corpus reruns are ANALYTICAL RECURRENCES; a registered multivariate null cannot erase secondary unconfirmed univariate candidates) | null-and-replication-language |

## Dated additions (2026-08-22, fortieth note — X5-T temporal transport; append-only)

| # | era / leg | where recorded | one-line description | rule it paid for | family |
|---|---|---|---|---|---|
| #96 | X-line / M4-X5-T | `docs/SUICA_CROSS_LEVEL_INFERENCE_CONTRACT.md`, `reports/SUICA_M4_X5_TEMPORAL_TRANSPORT_AUDIT.md` | A count over nested K=2/4/8 views could be mistaken for independent replication, and temporal stability of the pooled within-person slope could be mistaken for stability of the full cross-level transport family. At K=8 the disjoint cohort has 4/5 equivalent within paths but only R5 retains its historical family in all segments. | none (convention: nested resolutions are a sensitivity path, not replication; report the finest registered resolution separately; temporal-persistence and cross-level-transport gates remain independent; event-count segments are not called calendar-time intervals) | nested-resolution-and-gate-separation |
